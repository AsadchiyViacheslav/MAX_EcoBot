...
export async function handleEcoPlaces(ctx: Context, city: string, offset = 0) {
  const points = getPoints(city, 'ZW', offset, 10);
  if (!points.length) {
    const backKeyboard = Keyboard.inlineKeyboard([
      [Keyboard.button.callback('⬅️ Назад', 'back:main')]
    ]);
    return ctx.reply(`❌ В городе "${city}" нет эко-френдли заведений.`, { attachments: [backKeyboard] });
  }

  let message = `🌿 **Эко-френдли заведения в городе ${city}**\n\n`;
  points.forEach((p, i) => {
    const mats = translateMaterials(p.materials).join(', ');
    message += `**${i + 1 + offset}. ${p.name}**\n📍 ${p.address}\n♻️ Категории: ${mats}\n\n`;
  });

  const total = countPoints(city, 'ZW');

  const buttons = [[Keyboard.button.callback('⬅️ Назад', 'back:main')]];
  if (offset + points.length < total) {
    buttons.push([Keyboard.button.callback('➡️ Больше', `eco:${city}:${offset + 10}`)]);
  }
